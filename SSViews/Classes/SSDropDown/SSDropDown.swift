...
open class SSDropDown: UITextField, UIPickerViewDelegate, UIPickerViewDataSource {
	...
	fileprivate var _selectedItem: DropDownObject?
	...
	public var selectElement: DropDownObject? {
		didSet {
			for (index, element) in data.enumerated() {
				if selectElement?.getString() == element.getString() {
	...
	public func selectPicker(row: Int) {
	...
	//*****************************************************
	//End pickerView
	fileprivate func setSelectedItem(_ row: Int) {
		if data.count > 0 {
			self.text = data[row].getString()
			_selectedItem = data[row]
